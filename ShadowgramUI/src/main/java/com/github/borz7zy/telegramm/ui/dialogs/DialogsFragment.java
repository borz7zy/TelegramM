...
public class DialogsFragment extends BaseTelegramFragment implements Client.ResultHandler {
    ...
    private void rebuildDialogsFromCache() {
        ...
        for (TdApi.Chat chat : viewModel.getChatCache().values()) {
            ...
            if (order != 0) {
                viewModel.getDialogs().put(chat.id,
                        new DialogItem(AppManager.getInstance().getContext(), chat, order));
            }
        }
    }
    ...
    private void cacheChat(TdApi.Chat chat) {
        ...
        com.github.borz7zy.telegramm.core.theme.ThemeRepository.get().seedChat(chat);
    }
    ...
    private void updateChat(TdApi.Chat chat) {
        ...
        DialogItem newItem = new DialogItem(AppManager.getInstance().getContext(), chat, order);
